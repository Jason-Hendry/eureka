...
import {BlankDivisionResult, BlankResultPlaces, RaceData, ResultPlace} from "../../models/Race";
...
const divisionalResults = (data: RaceData, merge: (newValue: Partial<RaceData>)=>void) => {
    return <RepeatSection sortable={false} id={"scratchDivisions"} value={data.divisionResults || []} label={"Scratch Divisions"} onChange={v => merge({divisionResults: v || undefined})} section={(v,m) => (
        <>
...
const placeResults = (data: {results?: ResultPlace[]}, merge: (newValue: Partial<{ results: ResultPlace[] }>)=>void) => {
    return <RepeatSection sortable={false} id={"scratchDivisions"} value={data.results || []} label={"Place"} onChange={v => merge({results: v || undefined})} section={(v,m,i) => (
...
const AdminIndex: VFC = () => {
    ...
    const {save, merge, data: race, isEdit, errors} = useAdminEffects<RaceData>(RaceCollectionApi, () => ({Title: ""}), returnToList)

    useEffect(() => {
        console.log(hasDivisions(race?.RaceFormat))
        if(race && race.divisionResults === undefined && hasDivisions(race?.RaceFormat)) {
            merge({divisionResults: Array.from({length: 4}, (_,i) => ({
        ...
        if(race && race.results === undefined && !hasDivisions(race?.RaceFormat)) {
            merge({results: BlankResultPlaces(5)})
        }
    }, [merge, race])
    ...
    return <AdminForm label={`Race Results ${race.Date} ${race.Title} ${race.RaceFormat}`} deleteRecord={null} isEdit={isEdit} save={save} errors={errors}>
        {resultType}
    </AdminForm>
